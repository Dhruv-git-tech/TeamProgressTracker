...
def load_progress():
    try:
        ...
        pass  # If file is corrupted or unreadable, fall back to fresh
...
for user in TEAM_MEMBERS:
    if isinstance(progress_copy.get(user, None), str):
        # Convert old str to new dict format
...
# --- Member view ---
if name != ADMIN_USERNAME:
    st.markdown("<h3 style='color:#6a0572;'>Welcome AI Developer Interns!</h3>", unsafe_allow_html=True)
...
# --- Admin view ---
    ...
    import pandas as pd
    df = pd.DataFrame.from_dict(st.session_state['progress'], orient='index', columns=DAYS_OF_WEEK)
    st.dataframe(df, use_container_width=True)
    st.markdown('</div>', unsafe_allow_html=True)
    st.snow()
    # Confetti if all members have updated all days
    if all(all(st.session_state['progress'][m][d].strip() for d in DAYS_OF_WEEK) for m in TEAM_MEMBERS):
        st.success("🎉 All members have updated their weekly progress!")
    # Team completion chart (number of members who updated all days)
    completed = sum(1 for m in TEAM_MEMBERS if all(st.session_state['progress'][m][d].strip() for d in DAYS_OF_WEEK))
...
for member in TEAM_MEMBERS:
    ...
    st.markdown(f'<div style="background:{color};border-radius:0.5em;padding:0.5em 1em;margin-bottom:0.5em;"> <b>{member}:</b> {prog or "<i>No update yet</i>"} </div>', unsafe_allow_html=True)
